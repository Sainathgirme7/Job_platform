...
from .forms import UserRegistrationForm,HrRegistrationForm, UserLoginForm,  SetPasswordForm, PasswordResetForm
from .decorators import user_not_authenticated
from .tokens import account_activation_token    
from .models import SubscribedUsers
# i have add this because i got an error in my code User not defind.
...
def hrregister(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            # hr_user = Hr.objects.create(user=user)
            activateEmail(request, user, form.cleaned_data.get('email'))
            return redirect('homepage')
    ...
    return render(
        request=request,
        template_name="hrs/register.html",
        context={"form": form}
        )
        ...
        if form.is_valid():
            ...
            if user is not None:
                ...
                if Hr.objects.filter(user=user).exists():
                    request.session['is_hr'] = True
                else:
                    request.session['is_hr'] = False
                return redirect('homepage')
